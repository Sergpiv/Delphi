...
if combobox1.ItemIndex<>-1 then begin
 ...
 nalad.Showmodal;end
 else showmessage('Не выбран месяц?');
...
if not inputquery('Норма отработанного времени','Пожалуйста, введите'+#13+#10+'количество нормируемых'+#13+#10+'часов в данном месяце?',str) then exit else
...
decodedate(date,y,m,day);d:=encodedate(y,m,1);
  ...
  if combobox1.ItemIndex=11 then mod_dann.DataModule1.frxReport1.Variables['data_new'] :=''''+ a[1]+' месяц.'+'''' else
  mod_dann.DataModule1.frxReport1.Variables['data_new'] :=''''+ a[combobox1.ItemIndex+2]+' месяц.'+'''';
...
begin
if messagedlg('Вы уверены в удалении всех записей по зарплате?',mtWarning, mbOKCancel, 0)=mrok then
query(true);
end;
...
mod_dann.DataModule1.ADOquery9.Parameters[0].Value:=combobox1.ItemIndex+1;
mod_dann.DataModule1.ADOquery9.Active:=true;
str:='ЗАРПЛАТА БРИГАДЫ:                    *** '+floattostr(round(vipusk_zarplata(zarpl.Form1.ComboBox1.ItemIndex+1)/okrugl)*okrugl)+' руб. ***'+#10+#13+'_______________________________________________________'+#10+#13+'Выпуск  по  нарядам:                           ';
...
str:=str+#10+#13+'Незавершенка прошлого месяца:   ';
...
str:=str+#10+#13+'Незавершенка текущего месяца:      ';if round(nezavershenka(ComboBox1.ItemIndex+1)/okrugl)*okrugl<>0 then begin sum_vip:=sum_vip+round(nezavershenka(ComboBox1.ItemIndex+1)/okrugl)*okrugl;str:=str+floattostr(round(nezavershenka(ComboBox1.ItemIndex+1)/okrugl)*okrugl)+' руб.';end;
str:=str+#10+#13+'Брак:                                                          ';if brak_det(ComboBox1.ItemIndex+1)<>0 then begin sum_vip:=sum_vip+round(brak_det(ComboBox1.ItemIndex+1)/okrugl)*okrugl;str:=str+floattostr(round(brak_det(ComboBox1.ItemIndex+1)/okrugl)*okrugl)+' руб.';end;
str:=str+#10+#13+'_______________________________________________________'+#10+#13;
str:=str+#10+#13+'ИТОГО ВЫПУСК:                              *** '+floattostr(sum_vip)+' руб. ***';
str:=str+#10+#13+'ВЫПУСК БТЗ+НЕЗАВЕРШЕНКА:    *** ';if mod_dann.DataModule1.ADOTable_vip_korr.FieldByName('BTZ').Value<>null then sum_btz:=sum_btz+round(mod_dann.DataModule1.ADOTable_vip_korr.FieldByName('BTZ').Value/okrugl)*okrugl;
...
str:=str+#10+#13+'=================================='+#10+#13;
str:=str+'КОРРЕКТИВ:                                       *** '+floattostr(round((sum_vip-vipusk_zarplata(zarpl.Form1.ComboBox1.ItemIndex+1))/okrugl)*okrugl)+' руб. ***';
str:=str+#10+#13+'КОРРЕКТИВ БТЗ:                               *** '+floattostr(round((sum_btz-vipusk_zarplata(zarpl.Form1.ComboBox1.ItemIndex+1))/okrugl)*okrugl)+' руб. ***';
...
begin
if messagedlg('Вы уверены в удалении данных за наладку?',mtWarning, mbOKCancel, 0)=mrok then
begin
  mod_dann.DataModule1.ADOQuery_del.SQL.Clear;
...
begin
if messagedlg('Вы уверены в удалении всех данных за текущий месяц?',mtWarning, mbOKCancel, 0)=mrok then
begin
del_sql('Chasi');
del_sql('Korrectiv');
 ...
 if mod_dann.DataModule1.ADOTable_recvisit_dobavl.Locate('name_rec','BASE_PATH',[]) then
  if DirectoryExists(mod_dann.DataModule1.ADOTable_recvisit_dobavl.FieldByName('znach').AsString) then
   ...
   curr^.name:=FindFile.Name;
              curr^.next:=head;
              head:=curr;
              while FindNext(FindFile)=0 do begin  //Ищем другие файлы на диске, пока не произойдёт ошибка поиска
       new(curr); // выделить память для элемента списка
   curr^.name:=FindFile.Name;
              curr^.next:=head;
              head:=curr;
...
FDConnection.LoginPrompt:=false;
    ...
    adotable.Post;
     SQLiteQuery.Next;
        ...
        messagedlg('База '+myLookup+' успешно скопирована' ,mtinformation,[mbOK],0);
    ...
    moveFile(Pchar(mod_dann.DataModule1.ADOTable_recvisit_dobavl.FieldByName('znach').AsString+'\'+curr^.name),Pchar(mod_dann.DataModule1.ADOTable_recvisit_dobavl.FieldByName('znach').AsString+'\temp\temp_'+inttostr(combobox1.ItemIndex+1)+'_'+curr^.name));
                                        ...
                                        else showmessage('Нет записей в базе '+curr^.name+' по текущему месяцу для копирования!!!');
     ...
     fdConnection.Destroy
      ...
      else showmessage('Базы для загрузки не найдены!!!');
     end
      else begin showmessage('Каталог по указанному пути не существует!!!'+#13+#10+'Измените путь в реквизитах!!!');
      if not assigned(recvisit) then
recvisit:=trecvisit.Create(self);
recvisit.show;
      end
      else begin
      showmessage('Путь к каталогу не задан в настройках!!!');
            if not assigned(recvisit) then
recvisit:=trecvisit.Create(self);
recvisit.show;
      end;
...
form1.Caption:='Зарплата-unregistered version';regbool:=false;end else begin regbool:=true;form1.Caption:='Зарплата';end;
...
if not fileexists(F.ReadString('DIR','startupDir','')) then begin showmessage('Не найдена база данных. Укажите путь к базе данных.');exit;   end;
mod_dann.DataModule1.ADOConnection1.ConnectionString:='DSN=Зарплата; User ID=Пивовар Сергей;DBQ='+F.ReadString('DIR','startupDir','')+';DriverId=25;FIL=MS Access;MaxBufferSize=2048;PageTimeout=5;UID=admin;';
try
...
begin
mod_dann.DataModule1.ADOTable_rabotn.Locate('familia','Пивовар',[loPartialKey]);
combobox1.OnChange(sender);
vrema;
end;
...
AdoQuery1.ExecSQL;
zapros('ZECH','Цех');
zapros('UCHASTOK','Участок');
zapros('BRIGADA','Бригада');
zapros('UCH','Наменование участка');
zapros('RESH','Решение');
zapros('RABOCH','Рабочих в бригаде');
zapros('SOVET','В совете бригады');
zapros('PRIS','Присутствовало');
zapros('DOKL','Докладывал');
zapros('VIST','Выступали');
zapros('BRIGADIR','Бригадир');
zapros('NALADKA','Процент за наладку');
zapros('NORMA','Норма выработки за 8 ч.');
zapros('OKRUGL','Округлять зарплату до');
zapros('OKR_RASC','Округлять расценки до');
zapros('PROC_MAX_VIR','Процент максимальной выработки');
AdoQuery1.SQL.Clear;
...
s:='CREATE VIEW nezaversh AS SELECT raszenki.Cod_detali, raszenki.Zachod, raszenki.Nomer_operazii, raszenki_1.Raszenka, Sum([nezavershenka]![kol]) AS [sum-kol], [Sum-kol]*[raszenki_1]![Raszenka] AS Выражение1';
...
s:='CREATE VIEW bra AS SELECT raszenki.Cod_detali, raszenki.Zachod, raszenki.Nomer_operazii, raszenki_1.Raszenka, Sum([brak]![kol]) AS [sum-kol], [Sum-kol]*[raszenki_1]![Raszenka] AS Выражение1';
...
mod_dann.DataModule1.ADOConnection1.Execute(s);
 ...
 ShowMessage('База с таким именем уже существует!!!'+#13+'Сохраните базу под другим именем.');  n14.Tag:=0;
 end;
